...
class ScheduleParser(ABC):
    ...
    def validate_result(self, result: Dict[str, pd.DataFrame]) -> bool:
        ...
        if 'projects' not in result or 'activities' not in result:
            logger.error("Result must contain 'projects' and 'activities' keys")
            return False
        
        if not isinstance(result['projects'], pd.DataFrame):
            logger.error("'projects' must be a DataFrame")
            return False
        
        if not isinstance(result['activities'], pd.DataFrame):
            logger.error("'activities' must be a DataFrame")
            return False
        
        logger.info(f"Validation passed: {len(result['projects'])} projects, {len(result['activities'])} activities")
        return True
